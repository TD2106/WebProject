...
DELIMITER $$
USE `movieStreamingWebsite`$$
CREATE DEFINER = CURRENT_USER TRIGGER `movieStreamingWebsite`.`Member_AFTER_DELETE` AFTER DELETE ON `Member` FOR EACH ROW
BEGIN
...
USE `movieStreamingWebsite`$$
CREATE DEFINER = CURRENT_USER TRIGGER `movieStreamingWebsite`.`Admin_AFTER_DELETE` AFTER DELETE ON `Admin` FOR EACH ROW
BEGIN
	delete from Member where Member.idMember = old.idMember;
END$$

USE `movieStreamingWebsite`$$
CREATE DEFINER = CURRENT_USER TRIGGER `movieStreamingWebsite`.`Movie_AFTER_DELETE` AFTER DELETE ON `Movie` FOR EACH ROW
BEGIN
...
USE `movieStreamingWebsite`$$
CREATE DEFINER = CURRENT_USER TRIGGER `movieStreamingWebsite`.`Log_AFTER_DELETE` AFTER DELETE ON `Log` FOR EACH ROW
BEGIN
...
USE `movieStreamingWebsite`$$
CREATE DEFINER = CURRENT_USER TRIGGER `movieStreamingWebsite`.`WatchLog_AFTER_DELETE` AFTER DELETE ON `WatchLog` FOR EACH ROW
BEGIN
	delete from Log where Log.idLog = old.idLog;
END$$

USE `movieStreamingWebsite`$$
CREATE DEFINER = CURRENT_USER TRIGGER `movieStreamingWebsite`.`AdminLog_AFTER_DELETE` AFTER DELETE ON `AdminLog` FOR EACH ROW
BEGIN
	delete from Log where Log.idLog = old.idLog;
END$$
